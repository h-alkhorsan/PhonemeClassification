...
z1 = np.zeros((grid.shape[0], grid.shape[1], k))
z2 = np.zeros((grid.shape[0], grid.shape[1], k))

for i in range(grid.shape[0]):
    z1[i] = get_predictions(GMM_PARAMS_01['mu'], GMM_PARAMS_01['s'], GMM_PARAMS_01['p'], grid[i])
    z2[i] = get_predictions(GMM_PARAMS_02['mu'], GMM_PARAMS_02['s'], GMM_PARAMS_02['p'], grid[i])

M = np.zeros((grid.shape[0], grid.shape[1]))

for i in range(grid.shape[0]):
    for j in range(grid.shape[1]):
        if (np.sum(z1[i][j]) > np.sum(z2[i][j])):
            M[i][j] = 1
        else:
            M[i][j] = 2
M = M.T
